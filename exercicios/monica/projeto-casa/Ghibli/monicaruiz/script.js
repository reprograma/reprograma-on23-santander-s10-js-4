...
function createCard(movies) {
  return `
    <div class="filme">
      <img class="image" src="${movies.image}">
      <h2 class="title">${movies.title}</h2>
      <p class="original_title>${movies.director}</p>
      <p class="data">${movies.release_date}</p>
    </div>
  `} 
...
async function main() {
  ...
  films.map((films) => movies.innerHTML += createCard(films))
}
